...
public class MultiLineChartActivity extends DemoBase implements OnSeekBarChangeListener,
    ...
    public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
        ...
        for (int z = 0; z < 3; z++) {
            ...
            values.add(new Entry(0, (float) (Math.random() * seekBarY.getProgress()) + 3));

            for (int i = 1; i < progress; i++) {
                double val = (Math.random() * seekBarY.getProgress());
                values.add(new Entry(i, (float) (values.get(i-1).getY() + val)));
            }
        ...
        chart.setOnChartDataSetSelectedListener(new OnChartDataSetSelectedListener()
        {
            @Override
            public void onDataSetSelected(int dataSetIndex)
            {
                System.out.println("DATASET SELECTED " + dataSetIndex);
            }
